//检测是否为空
function check_null(value) {
  if ( value == "" ) {
    return true;
  }
  var r = new RegExp(/^[ ]+$/);

  var result = r.test(value);
  return result;
}

//检测手机
function check_phone(value) {
  var r = new RegExp(/^(1\d{10})$/);

  var result = r.test(value);
  return result;
}

//检测固话
function check_tel(value) {
  var r = new RegExp(/^((0\d{2,3}-\d{7,8}))$/);

  var result = r.test(value);
  return result;
}

//检测手机和固话
function check_telphone(value) {
  var r = new RegExp(/^((0\d{2,3}-\d{7,8})|(1\d{10}))$/);

  var result = r.test(value);
  return result;
}

//检测邮箱
function check_email(value) {
  var r = new RegExp(/^[\w!#$%&'*+/=?^_`{|}~-]+(?:\.[\w!#$%&'*+/=?^_`{|}~-]+)*@(?:[\w](?:[\w-]*[\w])?\.)+[\w](?:[\w-]*[\w])?$/);

  var result = r.test(value);
  return result;
}

//检测身份证
function check_idcard(value) {
  var r = new RegExp(/^[1-9]{1}[0-9]{14}$|^[1-9]{1}[0-9]{16}([0-9]|[xX])$/);

  var result = r.test(value);
  return result;
}

//检测姓名
function check_name(value) {
  var r = new RegExp(/^[a-zA-Z\u4e00-\u9fa5\_\-\.\·]*$/);

  var result = r.test(value);
  return result;
}

//检测姓名(中文)
function check_cn_name(value) {
  var r = new RegExp(/^[\u4e00-\u9fa5\.\·]*$/);

  var result = r.test(value);
  return result;
}

//检测姓名(英文)
function check_en_name(value) {
  var r = new RegExp(/^[a-zA-Z\_\-\.\·]*$/);

  var result = r.test(value);
  return result;
}

//检测用户名(长度在3~16之间，只能包含英文、数字和下划线，区分大小写)
function check_username(value) {
  var r = new RegExp(/^[a-zA-Z0-9_-]{3,16}$/);

  var result = r.test(value);
  return result;
}

//检测密码(以字母开头，长度在6~18之间，只能包含字符、数字和下划线，区分大小写)
function check_password(value) {
  var r = new RegExp(/^[a-zA-Z]\w{5,17}$/);

  var result = r.test(value);
  return result;

}

//检测url
function check_url(value) {
  var r = new RegExp(/^(https?:\/\/)?([\da-z\.-]+)\.([a-z\.]{2,6})([\/\w \.-]*)*\/?$/);

  var result = r.test(value);
  return result;
}

//检测ip地址
function check_ip(value) {
  var r = new RegExp(/^(\d{1,2}|1\d\d|2[0-4]\d|25[0-5])\.(\d{1,2}|1\d\d|2[0-4]\d|25[0-5])\.(\d{1,2}|1\d\d|2[0-4]\d|25[0-5])\.(\d{1,2}|1\d\d|2[0-4]\d|25[0-5])$/);

  var result = r.test(value);
  return result;
}

//检测金钱格式(两位小数)
function check_money(value) {
  var r = new RegExp(/(^[1-9](\d+)?(\.\d{1,2})?$)|(^(0){1}$)|(^\d\.\d{1,2}?$)/g);

  var result = r.test(value);
  return result;
}

//检测整数
function check_int(value) {
  var r=new RegExp(/^[0-9]\d*$/);

  var result = r.test(value);
  return result;
}

//检测正整数
function check_int2(value) {
  var r=new RegExp(/^[1-9]\d*$/);

  var result = r.test(value);
  return result;
}

//emoji表情验证
function check_emoji(value) {
  var r=new RegExp(/^([\u00A9\u00AE\u203C\u2049\u2122\u2139\u2194-\u2199\u21A9-\u21AA\u231A-\u231B\u2328\u23CF\u23E9-\u23F3\u23F8-\u23FA\u24C2\u25AA-\u25AB\u25B6\u25C0\u25FB-\u25FE\u2600-\u2604\u260E\u2611\u2614-\u2615\u2618\u261D\u2620\u2622-\u2623\u2626\u262A\u262E-\u262F\u2638-\u263A\u2640\u2642\u2648-\u2653\u2660\u2663\u2665-\u2666\u2668\u267B\u267F\u2692-\u2697\u2699\u269B-\u269C\u26A0-\u26A1\u26AA-\u26AB\u26B0-\u26B1\u26BD-\u26BE\u26C4-\u26C5\u26C8\u26CE-\u26CF\u26D1\u26D3-\u26D4\u26E9-\u26EA\u26F0-\u26F5\u26F7-\u26FA\u26FD\u2702\u2705\u2708-\u270D\u270F\u2712\u2714\u2716\u271D\u2721\u2728\u2733-\u2734\u2744\u2747\u274C\u274E\u2753-\u2755\u2757\u2763-\u2764\u2795-\u2797\u27A1\u27B0\u27BF\u2934-\u2935\u2B05-\u2B07\u2B1B-\u2B1C\u2B50\u2B55\u3030\u303D\u3297\u3299]|\uD83C[\uDC04\uDCCF\uDD70-\uDD71\uDD7E-\uDD7F\uDD8E\uDD91-\uDD9A\uDDE6-\uDDFF\uDE01-\uDE02\uDE1A\uDE2F\uDE32-\uDE3A\uDE50-\uDE51\uDF00-\uDF21\uDF24-\uDF93\uDF96-\uDF97\uDF99-\uDF9B\uDF9E-\uDFF0\uDFF3-\uDFF5\uDFF7-\uDFFF])|(\uD83D[\uDC00-\uDCFD\uDCFF-\uDD3D\uDD49-\uDD4E\uDD50-\uDD67\uDD6F-\uDD70\uDD73-\uDD7A\uDD87\uDD8A-\uDD8D\uDD90\uDD95-\uDD96\uDDA4-\uDDA5\uDDA8\uDDB1-\uDDB2\uDDBC\uDDC2-\uDDC4\uDDD1-\uDDD3\uDDDC-\uDDDE\uDDE1\uDDE3\uDDE8\uDDEF\uDDF3\uDDFA-\uDE4F\uDE80-\uDEC5\uDECB-\uDED2\uDEE0-\uDEE5\uDEE9\uDEEB-\uDEEC\uDEF0\uDEF3-\uDEF6])|(\uD83E[\uDD10-\uDD1E\uDD20-\uDD27\uDD30\uDD33-\uDD3A\uDD3C-\uDD3E\uDD40-\uDD45\uDD47-\uDD4B\uDD50-\uDD5E\uDD80-\uDD91\uDDC0])$/);

  var result = r.test(value);
  return result;
}


module.exports.check_null = check_null
module.exports.check_phone = check_phone
module.exports.check_tel = check_tel
module.exports.check_telphone = check_telphone
module.exports.check_email = check_email
module.exports.check_idcard = check_idcard
module.exports.check_name = check_name
module.exports.check_cn_name = check_cn_name
module.exports.check_en_name = check_en_name
module.exports.check_username = check_username
module.exports.check_password = check_password
module.exports.check_url = check_url
module.exports.check_ip = check_ip
module.exports.check_money = check_money
module.exports.check_int = check_int
module.exports.check_int2 = check_int2
module.exports.check_emoji = check_emoji

